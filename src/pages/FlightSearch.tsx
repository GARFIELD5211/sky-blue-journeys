          ...
          <div className="glass-card rounded-2xl p-5 md:p-6 mb-8">
            <div className="flex flex-wrap items-center gap-4 mb-4">
              {[
                ...
                <label key={t.value} className="flex items-center gap-1.5 cursor-pointer text-sm">
                  <input type="radio" name="tripType" value={t.value} checked={tripType === t.value} onChange={() => setTripType(t.value)} className="accent-primary" />
                  <span className={tripType === t.value ? "font-semibold text-foreground" : "text-muted-foreground"}>{t.label}</span>
                </label>
              ))}
              <Select value={flightClass} onValueChange={setFlightClass}>
                <SelectTrigger className="w-32 bg-background/50 h-8 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
              <div className="md:col-span-3 space-y-1">
                <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1"><PlaneTakeoff className="w-3.5 h-3.5" /> Flying From</label>
                <Select value={from} onValueChange={setFrom}>
                  <SelectTrigger className="bg-accent/40 h-11"><SelectValue placeholder="Select origin" /></SelectTrigger>
                  <SelectContent>{uniqueFroms.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="md:col-span-1 flex justify-center">
                <Button variant="outline" size="icon" onClick={swapCities} className="rounded-full h-9 w-9 border-primary/30"><ArrowRightLeft className="w-4 h-4 text-primary" /></Button>
              </div>
              <div className="md:col-span-3 space-y-1">
                <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1"><PlaneTakeoff className="w-3.5 h-3.5 rotate-90" /> Flying To</label>
                <Select value={to} onValueChange={setTo}>
                  <SelectTrigger className="bg-accent/40 h-11"><SelectValue placeholder="Select destination" /></SelectTrigger>
                  <SelectContent>{uniqueTos.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              ...
              <div className="md:col-span-2 space-y-1">
                <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1"><Users className="w-3.5 h-3.5" /> Travellers</label>
              ...
              <div className="md:col-span-1">
                <Button className="w-full h-11 gradient-primary border-0 text-primary-foreground hover:opacity-90"><Search className="w-5 h-5" /></Button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 mt-3 pt-3 border-t border-border/50">
              {[
                ...
                <div key={t.label} className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => t.set(Math.max(t.min, t.value - 1))} disabled={t.value <= t.min}><Minus className="w-3 h-3" /></Button>
                  <span className="text-sm font-semibold w-4 text-center text-foreground">{t.value}</span>
                  <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => t.set(t.value + 1)}><Plus className="w-3 h-3" /></Button>
                </div>
              ))}
            </div>
          </div>
            ...
            <aside className="lg:w-64 shrink-0 space-y-6">
              <div className="glass-card rounded-2xl p-5">
                ...
                <label className="flex items-center gap-2 mb-2 text-sm cursor-pointer">
                  <Checkbox checked={filterAirlines.length === 0} onCheckedChange={() => setFilterAirlines([])} />
                  <span className="text-foreground">All</span>
                </label>
                {uniqueAirlines.map(a => (
                  <label key={a} className="flex items-center gap-2 mb-2 text-sm cursor-pointer">
                    <Checkbox checked={filterAirlines.includes(a)} onCheckedChange={() => toggleAirline(a)} />
              ...
              <div className="glass-card rounded-2xl p-5">
                ...
                {["cheapest", "fastest"].map(s => (
                  <label key={s} className="flex items-center gap-2 mb-2 text-sm cursor-pointer">
                    <Checkbox checked={filterSort === s} onCheckedChange={() => setFilterSort(filterSort === s ? "" : s)} />
              ...
              <div className="glass-card rounded-2xl p-5">
                ...
                {[
                  ...
                  <label key={s.value} className="flex items-center gap-2 mb-2 text-sm cursor-pointer">
                    <Checkbox checked={filterStops === s.value} onCheckedChange={() => setFilterStops(s.value)} />
            ...
            {isLoading ? (
              ...
              filtered.map((flight, i) => (
                <ScrollReveal key={i} delay={i * 0.05}>
                  <div className="glass-card rounded-2xl p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-4 md:gap-6 hover:shadow-lg transition-shadow">
                    ...
                    <div className="text-center md:text-right md:w-36 shrink-0">
                      ...
                      <Button size="sm" className="mt-2 gradient-primary border-0 text-primary-foreground hover:opacity-90 text-xs px-6">Select Now</Button>
                    </div>
                  </div>
                </ScrollReveal>
              ))
